...
<html>
  ...
  <script type="module">
    ...
    import { SocketIo } from "../../libs/client-socket-io/dist/index.js";
    const socket = new SocketIo("ws://127.0.0.1:3000/socket");
    await socket.connect();
    
    async function Action(msg) {
      if (msg.startsWith(":ping")) socket.notify("ping", msg.slice(5));
    }

    async function onPong() {
      for await (const msg of socket.on("pong")) {
        appendMsg("[Pong] " + new TextDecoder().decode(msg));
      }
    }
